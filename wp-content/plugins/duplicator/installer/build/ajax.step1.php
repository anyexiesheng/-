...
if (isset($_GET['dbtest'])) {
	...
	$tstSrv   = ($dbConn)  ? "<div class='dup-pass'>成功</div>" : "<div class='dup-fail'>失败</div>";
	$tstDB    = ($dbFound) ? "<div class='dup-pass'>成功</div>" : "<div class='dup-fail'>失败</div>";
	$html	 .= "<div class='dup-db-test'>";
	$html	 .= "<small style='display:block; padding:5px'>使用链接字符:<br/>服务器={$_POST['dbhost']}; 数据库={$_POST['dbname']}; Uid={$_POST['dbuser']}; Pwd={$_POST['dbpass']}; Port={$_POST['dbport']}</small>";
	$html	 .= "<label>服务器已连接:</label> {$tstSrv} <br/>";
	$html	 .= "<label>数据库已找到:</label>   {$tstDB} <br/>";
...
$log .= "存档设置\n";
$log .= "********************************************************************************\n";
$log .= "名称:\t{$_POST['package_name']}\n";
$log .= "大小:\t" . DupUtil::readable_bytesize(@filesize($_POST['package_name'])) . "\n";
