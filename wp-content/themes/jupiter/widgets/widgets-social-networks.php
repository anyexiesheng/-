...
class Artbees_Widget_Social extends WP_Widget {
	...
	function Artbees_Widget_Social() {
		$widget_ops = array( 'classname' => 'widget_social_networks', 'description' => '显示社会化网络图标列表' );
		$this->WP_Widget( 'social', THEME_SLUG.' - '.'社会化网络', $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( !empty( $instance['enable_sites'] ) ) {
			foreach ( $instance['enable_sites'] as $site ) {
				...
				if($site=="weibo"){$nsite="新浪微博";}else if($site=="weixin"){$nsite="微信";}else if($site=="qzone"){$nsite="QQ空间";}else if($site=="renren"){$nsite="人人网";}else{$nsite=$site;};
				$link = isset( $instance[strtolower( $site )] )?$instance[strtolower( $site )]:'#';
					$output .= '<a href="'.$link.'" rel="nofollow" class="builtin-icons '.$icon_style_css.$skin.' '.$path.' '.$site.'-hover" target="_blank" alt="'.$alt.' '.$nsite.'" title="'.$alt.' '.$nsite.'"><i class="'.$icon_style.$site.'"></i></a>';
...
?>
		...
		<p>
			<em><?php "注意: 请输入绝对地址 <br/>(e.g. <code>http://www.facebook.com/username</code>)";?></em>
		</p>
		...
		<?php for ( $i=1;$i<=10;$i++ ): $custom_name='custom_'.$i.'_name';$custom_url='custom_'.$i.'_url'; $custom_icon='custom_'.$i.'_icon'; ?>
			<div class="social_icon_custom_<?php echo $i;?>" <?php if ( $i>$custom_count ):?>style="display:none"<?php endif;?>>
				<p><label for="<?php echo $this->get_field_id( $custom_name ); ?>"><?php printf( '自定义%s名称:', $i );?></label>
				...
				<p><label for="<?php echo $this->get_field_id( $custom_url ); ?>"><?php printf( '自定义%s地址:', $i );?></label>
				...
				<p><label for="<?php echo $this->get_field_id( $custom_icon ); ?>"><?php printf( '自定义%s图标:', $i );?></label>
