...
function mk_portfolio_masonry_loop(&$r, $atts)
{
    ...
    if ($hover_scenarios == 'fadebox') {
        ...
        if ($post_type == 'image' || $post_type == '') {
            ...
            }elseif ($video_site == 'youku') {
               $video_url = 'http'.((is_ssl())? 's' : '').'://player.youku.com/embed/' . $video_id;
			}elseif ($video_site == 'tudou') {
				   $video_url = 'http'.((is_ssl())? 's' : '').'://www.tudou.com/programs/view/html5embed.action?code=' . $video_id;
			 }elseif ($video_site == 'wule') {
				   $video_url = 'http'.((is_ssl())? 's' : '').'://www.56.com/iframe/' . $video_id;
			 }elseif ($video_site == 'vqq') {
				   $video_url = 'http'.((is_ssl())? 's' : '').'://v.qq.com/iframe/player.html?vid=' . $video_id;				
    ...
    $output .= ($hover_scenarios == 'none') ? '</a><!-- project load -->' : '';
    $output .= '</div>';
