...
function blog_modern_style($atts, $current)
{
    ...
    if ($post_type == 'video') {
		...
		if ($video_site == 'youku') {
            $output .= '<div class="mk-video-wrapper"><div class="mk-video-container"><iframe src="http'.((is_ssl())? 's' : '').'://player.youku.com/embed/' . $video_id . '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div></div>';
        }
		if ($video_site == 'tudou') {
            $output .= '<div class="mk-video-wrapper"><div class="mk-video-container"><iframe src="http'.((is_ssl())? 's' : '').'://www.tudou.com/programs/view/html5embed.action?code=' . $video_id . '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div></div>';
        }
		if ($video_site == 'wule') {
            $output .= '<div class="mk-video-wrapper"><div class="mk-video-container"><iframe src="http'.((is_ssl())? 's' : '').'://www.56.com/iframe/' . $video_id . '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div></div>';
        }
		if ($video_site == 'vqq') {
            $output .= '<div class="mk-video-wrapper"><div class="mk-video-container"><iframe src="http'.((is_ssl())? 's' : '').'://v.qq.com/iframe/player.html?vid=' . $video_id . '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div></div>';
        }
        if ($video_site == 'vimeo') {
    ...
    if ($disable_comments_share != 'false') {
        ...
        if ($mk_options['single_blog_social'] == 'true'):
            ...
            $output .= '<li><a class="weibo-share" href="http://www.jiathis.com/send/?webid=tsina&url='.get_permalink().'&title='.get_the_title().'" target="_blank"><i class="mk-jupiter-icon-simple-weibo"></i></a></li>';
            $output .= '<li><a class="wenxin-share" href="http://www.jiathis.com/send/?webid=weixin&url='.get_permalink().'&title='.get_the_title().'" target="_blank"><i class="mk-jupiter-icon-simple-weixin"></i></a></li>';
            $output .= '<li><a class="qzone-share" href="http://www.jiathis.com/send/?webid=qzone&url='.get_permalink().'&title='.get_the_title().'" target="_blank"><i class="mk-jupiter-icon-simple-qzone"></i></a></li>';
            $output .= '<li><a class="renren-share" href="http://www.jiathis.com/send/?webid=renren&url='.get_permalink().'&title='.get_the_title().'" target="_blank"><i class="mk-jupiter-icon-simple-renren"></i></a></li>';
    ...
    $output .= '<div class="mk-blog-meta">'; 
        ...
        $output .= '<a class="blog-modern-btn" href="' . get_permalink() . '">' . __('Read More', 'mk_framework') . '</a>';
